...
def check_cfg(cfg, hard=True):
    ...
    # YOLOMM EXTENSION: Validate modality parameter for RGB+X framework
    if 'modality' in cfg and cfg['modality'] is not None:
        # Allow any string modality for flexible RGB+X framework
        if not isinstance(cfg['modality'], str) or not cfg['modality'].strip():
            raise ValueError(f"Invalid modality '{cfg['modality']}'. Modality must be a non-empty string (e.g., 'rgb', 'depth', 'thermal', 'ir', 'lidar', etc.)")
...
# Common modality examples for RGB+X multimodal framework
# Users can specify any modality string for flexible X-modal support
COMMON_MODALITIES = {'rgb', 'depth', 'thermal', 'ir', 'lidar', 'multispectral', 'hyperspectral'}
